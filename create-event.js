...
if (args.remain.length < 2) {
  throw new Error(
    'An event name and date are required. For example: \nnpm run create "Event Name" "2017-02-28 19:30"'
  );
}
...
if (!eventDate.isValid()) {
  throw new Error(
    `"${eventDateString}" is an invalid date. Please pass a date in the format "YYYY-MM-DD HH:mm".`
  );
}
const eventPath = path.join(
  ...
  'src',
  'events',
  String(eventDate.year()),
  Case.kebab(eventName)
);
...
function generateFile(fileName, content = '') {
  ...
  fs.stat(filePath, err => {
    if (err) {
      fs.writeFile(filePath, content, undefined, () =>
