  ...
  TimelineJS3 = {
    Timeline: class TestTimeline {
      constructor(title, config) {
        ...
        this.removeId = id => {
          this.config.events = this.config.events.filter(
            event => event.unique_id !== id
          );
        };
      }
    },
...
export default class Timeline {
  ...
  _filterEvent(event) {
    ...
    // Filter by tags
    return (
      event.tags.length === 0 || some(event.tags, tag => this.tags.has(tag))
    );
  }

  _updateEvents() {
    const futureEvents = this._filterEvents(this.events);
    const eventsToAdd = differenceWith(
      futureEvents,
      this._currentEvents(),
      eventsComparator
    );
    const eventsToRemove = differenceWith(
      this._currentEvents(),
      futureEvents,
      eventsComparator
    );
